...
--外挂有两个程序, 一个外挂,  一个控制台, 控制台用来控制所有外挂.
--通过网络进行通信, 外挂开机自动启动, 广播局域网, 控制台收到后把這個外掛當作一個客戶端
--控制台需要写个界面, 外挂命令行就行
...
日常脚本的伪代码
    ...
    (抓贵类似 不通过匹配当前任务驱动的脚本, 上面任务脚本是通过匹配屏幕来驱动执行的
游戏内有一种不需要任务来驱动执行, 比如做日常, 如果日常任务会显示在屏幕上也可以当作任务来写脚本
如果没有, 一种方式是找内存, 一种方式是从屏幕其他地方匹配. 比如日常的次数这种)
...
function 这就上船()
   ...
   对话();
   点击对话框内图片("这就上船啦");  --这和点击图片功能类似, 单独封装是因为对话框的点击需要让游戏处于焦点状态, 
